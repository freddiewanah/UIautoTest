...
class Device(object):
    ...
    def take_screenshot(self):
        ...
        #local_video_path = os.path.join(local_image_dir, "screen_%s.mp4" % tag)
        #remote_video_path = "/sdcard/screen_%s.mp4" % tag
        #self.adb.shell("screenrecord --time-limit %d %s" % (2, remote_video_path))
        #self.pull_file(remote_video_path, local_video_path)
        #self.adb.shell("rm %s" % remote_video_path)
    ...
    def take_video(self):
        ...
        local_image_path = os.path.join(local_image_dir, "screen_%s.mp4" % tag)
        remote_image_path = "/sdcard/screen_%s.mp4" % tag
        self.adb.shell("screenrecord --time-limit %d %s" % (3, remote_image_path))
        self.pull_file(remote_image_path, local_image_path)
        self.adb.shell("rm %s" % remote_image_path)
        self.video_path = local_image_path
    ...
    def get_current_state(self):
        ...
        try:
            ...
            current_state = DeviceState(self,
                                        ...
                                        videorecord_path=videorecord_path)
